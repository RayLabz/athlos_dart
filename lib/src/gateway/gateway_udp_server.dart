import 'dart:io';
import 'dart:typed_data';

import '../server/udp_server.dart';
import 'gateway_relay.dart';
import 'model/gateway_transport.dart';
import 'protocol/gateway_opcode.dart';
import 'protocol/gateway_packet.dart';

/// Gateway endpoint that accepts UDP clients and returns routing decisions.
class GatewayUdpServer {
  final UdpServer server;
  final GatewayRelay relay;

  GatewayUdpServer._({required this.server, required this.relay});

  factory GatewayUdpServer({required int port, required GatewayRelay relay}) {
    late final UdpServer server;

    server = UdpServer(
      name: 'AthlosGatewayUDP',
      port: port,
      onMessage: (message, address, clientPort) {
        _handleMessage(
          server: server,
          relay: relay,
          address: address,
          port: clientPort,
          bytes: message,
        );
      },
      onClientDisconnected: (client) {
        relay.disconnect(client.key);
      },
      logger: relay.logger,
    );

    return GatewayUdpServer._(server: server, relay: relay);
  }

  Future<void> start() async {
    await relay.start();
    await server.start();
  }

  void close() => server.close();

  static void _handleMessage({
    required UdpServer server,
    required GatewayRelay relay,
    required InternetAddress address,
    required int port,
    required Uint8List bytes,
  }) {
    final packet = GatewayPacket.tryParse(bytes);
    final clientKey = '${address.address}:$port';

    if (packet == null) {
      server.send(
        GatewayPacket(
          opcode: GatewayOpcode.error,
          data: {'message': 'Invalid packet.'},
        ).toBytes(),
        address,
        port,
      );
      return;
    }

    switch (packet.opcode) {
      case GatewayOpcode.authenticate:
        final token = packet.data['token'] as String?;
        if (token == null || token.isEmpty) {
          _sendError(server, address, port, 'Token is required.');
          return;
        }

        relay.authenticate(clientKey: clientKey, token: token).then((result) {
          if (!result.ok) {
            server.send(
              GatewayPacket(
                opcode: GatewayOpcode.authenticationFailed,
                data: {'message': result.error},
              ).toBytes(),
              address,
              port,
            );
            return;
          }

          server.send(
            GatewayPacket(
              opcode: GatewayOpcode.authenticated,
              data: {'playerId': result.playerId},
            ).toBytes(),
            address,
            port,
          );
        });
      case GatewayOpcode.matchmakingRequest:
        relay
            .createOrRefreshRoute(
              clientKey: clientKey,
              transport: GatewayTransport.udp,
            )
            .then((result) {
              if (!result.ok || result.route == null) {
                _sendError(
                  server,
                  address,
                  port,
                  result.error ?? 'Routing failed.',
                );
                return;
              }

              server.send(_routePacket(result.route!).toBytes(), address, port);
            });
      case GatewayOpcode.routeRequest:
        final sessionId = packet.data['sessionId'] as String?;
        if (sessionId == null || sessionId.isEmpty) {
          _sendError(server, address, port, 'sessionId is required.');
          return;
        }

        final result = relay.routeInfo(sessionId);
        if (!result.ok || result.route == null) {
          _sendError(server, address, port, result.error ?? 'Route not found.');
          return;
        }

        server.send(_routePacket(result.route!).toBytes(), address, port);
      default:
        _sendError(
          server,
          address,
          port,
          'Unsupported opcode for UDP gateway: ${packet.opcode.wireValue}',
        );
    }
  }

  static GatewayPacket _routePacket(route) {
    return GatewayPacket(
      opcode: GatewayOpcode.routed,
      data: {
        'sessionId': route.session.id,
        'playerId': route.session.playerId,
        'backend': {
          'id': route.backend.id,
          'host': route.backend.host,
          'port': route.backend.port,
        },
      },
    );
  }

  static void _sendError(
    UdpServer server,
    InternetAddress address,
    int port,
    String message,
  ) {
    server.send(
      GatewayPacket(
        opcode: GatewayOpcode.error,
        data: {'message': message},
      ).toBytes(),
      address,
      port,
    );
  }
}
